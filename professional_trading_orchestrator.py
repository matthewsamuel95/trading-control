...
class SupervisorOrchestrator:
    ...
    async def run_trading_cycle(self, initial_state: Optional[TradingState] = None) -> TradingState:
    ...
    async def _analysis_node(self, state: TradingState) -> TradingState:
        ...
        try:
            ...
            if analysis_result.get("signals"):
                state["proposed_trade"] = analysis_result["signals"][0]  # Take top signal
    ...
    async def _execution_node(self, state: TradingState) -> TradingState:
        ...
        try:
            if state["proposed_trade"]:
                ...
                execution_result = await self.execution_agent.execute_trade(state["proposed_trade"])
    ...
    async def _make_final_trade_decision(self, state: TradingState) -> Dict[str, Any]:
        ...
        if confidence > 0.8 and risk_score < 0.3:
            ...
            return {"execute": False, "reasoning": "Insufficient confidence or excessive risk"}
    
    def _add_experience_to_buffer(self, state: TradingState, execution_result: Dict[str, Any]):
...
class RiskControlAgent:
    ...
    async def assess_trade_risk(self, proposed_trade: Dict[str, Any], state: TradingState) -> Dict[str, Any]:
        ...
        risk_score = (trade_size / portfolio_value) * 0.5 + (current_positions / 10) * 0.3
        
        risk_snapshot = {
            "current_risk": risk_score,
            "portfolio_exposure": trade_size / portfolio_value if portfolio_value > 0 else 0,
